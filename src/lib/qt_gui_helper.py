...
def qlist_selected_labels(qlist):
    return [i.text() for i in qlist.selectedItems()]
...
def qtable_get_selected_columns(qtable):
    cols = [idx.column() for idx in qtable.selectionModel().selectedColumns()]
    return [qtable.horizontalHeaderItem(c).text() for c in cols]
